...
import { RTK_BIN, vmBuildRtk, vmEnsureReady, vmExec } from "./lib/vm";
...
const phaseArg = args.includes("--phase")
	...
	: null;
const phaseOnly =
	phaseArg !== null && !Number.isNaN(phaseArg) ? phaseArg : null;
...
const branch = (
	await $`git -C ${PROJECT_ROOT} branch --show-current`.text()
).trim();
...
if (buildInfo.binarySize < sizeLimit) {
	console.log(
		`  \x1b[32mPASS\x1b[0m | binary size | ${buildInfo.binarySize} bytes < 8MB`,
	);
} else {
	console.log(
		`  \x1b[31mFAIL\x1b[0m | binary size | ${buildInfo.binarySize} bytes >= 8MB`,
	);
}
...
if (shouldRun(3)) {
	...
	await testCmd(
		"files:diff",
		`${RTK} diff /home/ubuntu/rtk/src/main.rs /home/ubuntu/rtk/src/utils.rs`,
	);
	...
	await testCmd(
		"gh:pr list",
		`cd /home/ubuntu/rtk && ${RTK} gh pr list`,
		"any",
	);
	...
	await testCmd(
		"cargo:build",
		`export PATH=$HOME/.cargo/bin:$PATH && cd /tmp/test-rust && ${RTK} cargo build`,
	);
	await testCmd(
		"cargo:test",
	...
	await testCmd(
		"cargo:clippy",
		`export PATH=$HOME/.cargo/bin:$PATH && cd /tmp/test-rust && ${RTK} cargo clippy`,
	);
	...
	await testCmd(
		"python:ruff check",
		`cd /tmp/test-python && ${RTK} ruff check .`,
		1,
	);
	...
	await testCmd(
		"go:test",
		`export PATH=$PATH:/usr/local/go/bin && cd /tmp/test-go && ${RTK} go test ./...`,
		1,
	);
	await testCmd(
		"go:build",
		`export PATH=$PATH:/usr/local/go/bin && cd /tmp/test-go && ${RTK} go build .`,
		1,
	);
	await testCmd(
		"go:vet",
		`export PATH=$PATH:/usr/local/go/bin && cd /tmp/test-go && ${RTK} go vet ./...`,
		1,
	);
	await testCmd(
		"go:golangci-lint",
		`export PATH=$PATH:/usr/local/go/bin:$HOME/go/bin && cd /tmp/test-go && ${RTK} golangci-lint run`,
		1,
	);
	...
	await testCmd(
		"lint:eslint",
		`cd /tmp/test-node && ${RTK} lint 'eslint src/'`,
		"any",
	);
	await testCmd(
		"lint:prettier",
		`cd /tmp/test-node && ${RTK} prettier --check src/`,
		"any",
	);
	...
	await testCmd(
		"dotnet:build",
		`export DOTNET_ROOT=/usr/local/share/dotnet && export PATH=$PATH:$DOTNET_ROOT && cd /tmp/test-dotnet/TestApp 2>/dev/null && ${RTK} dotnet build || echo 'dotnet skip'`,
		"any",
	);
...
if (shouldRun(5)) {
	...
	await testCmd(
		"rewrite:export (no rewrite)",
		`${RTK} rewrite 'export FOO=bar'`,
		1,
	);
}
...
if (shouldRun(6)) {
	...
	await testCmd(
		"exit:git status=0",
		`cd /tmp/test-git && ${RTK} git status`,
		0,
	);
...
if (shouldRun(7)) {
	...
	await testSavings("savings:swiftlint", "swiftlint", `${RTK} swiftlint`, 20);
}
...
if (shouldRun(8)) {
	...
	await testCmd(
		"pipe:git status|wc",
		`cd /tmp/test-git && ${RTK} git status | wc -l`,
	);
	await testCmd("pipe:ls|wc", `${RTK} ls /home/ubuntu/rtk/src/ | wc -l`);
	await testCmd(
		"pipe:grep|head",
		`${RTK} grep 'fn' /home/ubuntu/rtk/src/ | head -5`,
	);
}
...
if (shouldRun(9)) {
	...
	await testCmd(
		"edge:grep NOTFOUND",
		`${RTK} grep NOTFOUND_XYZ /home/ubuntu/rtk/src/`,
		1,
	);
	await testCmd(
		"edge:unicode",
		`echo 'hello world' > /tmp/uni.txt && ${RTK} grep 'hello' /tmp`,
		"any",
	);
}
