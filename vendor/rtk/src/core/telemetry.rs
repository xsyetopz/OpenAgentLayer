...
use crate::core::utils::bytes_to_hex;
...
pub fn maybe_ping() {
    ...
    if var("RTK_TELEMETRY_DISABLED").unwrap_or_default() == "1" {
        return;
    }
...
fn get_or_create_salt() -> String {
    CACHED_SALT
        .get_or_init(|| {
            ...
            if let Ok(contents) = read_to_string(&salt_path) {
            ...
            if let Ok(mut f) = File::create(&salt_path) {
...
fn random_salt() -> String {
    ...
    if getrandom::fill(&mut buf).is_err() {
        ...
        return sha256_hex(fallback.as_bytes());
    }
    bytes_to_hex(buf)
}
...
fn count_custom_toml_filters() -> usize {
    ...
    if let Ok(cwd) = current_dir() {
        if let Ok(entries) = read_dir(cwd.join(".rtk/filters")) {
            count += entries
                .filter_map(|e| e.ok())
    ...
    if let Some(config_dir) = dirs::config_dir() {
        if let Ok(entries) = read_dir(config_dir.join("rtk/filters")) {
            count += entries
                .filter_map(|e| e.ok())
                .filter(|e| e.path().extension().is_some_and(|ext| ext == "toml"))
                .count();
...
fn detect_install_method() -> &'static str {
    ...
    let real_path = canonicalize(&exe)
        .unwrap_or(exe)
        .to_string_lossy()
        .to_string();
