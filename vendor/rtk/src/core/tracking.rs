...
fn opportunity_classification(
    _original_cmd: &str,
    rtk_cmd: &str,
    saved_tokens: usize,
    savings_pct: f64,
) -> (String, String) {
    if rtk_cmd.contains("rtk proxy") {
        return (
            "proxy".to_string(),
            "add native filter or safe rewrite".to_string(),
        );
    }
    if rtk_cmd.starts_with("rtk fallback:") {
        return (
            "fallback".to_string(),
            "add rewrite rule or TOML filter".to_string(),
        );
    }
    if rtk_cmd.contains("passthrough") {
        return (
            "passthrough".to_string(),
            "make existing route filter this case".to_string(),
        );
    }
    if saved_tokens == 0 || savings_pct <= 1.0 {
        return (
            "zero_savings".to_string(),
            "add filter coverage".to_string(),
        );
    }
    (
        "low_savings".to_string(),
        "tune existing filter".to_string(),
    )
}
...
/// Command group that most limits future token savings.
///
/// Returned by `rtk gain --opportunities` to identify filters or rewrite rules
/// worth improving next.
#[derive(Debug, Serialize)]
pub struct GainOpportunity {
    /// Command as originally requested.
    pub original_cmd: String,
    /// RTK command or fallback path that ran.
    pub rtk_cmd: String,
    /// Number of matching executions.
    pub count: usize,
    /// Total raw output token estimate.
    pub input_tokens: usize,
    /// Total filtered output token estimate.
    pub output_tokens: usize,
    /// Total tokens already saved.
    pub saved_tokens: usize,
    /// Weighted savings percentage for this command group.
    pub savings_pct: f64,
    /// Additional tokens recoverable if this group reached the target savings.
    pub recoverable_tokens: usize,
    /// Why this command is listed.
    pub reason: String,
...
impl Tracker {
    ...
    /// Rank command groups by tokens recoverable at the target savings rate.
    pub fn get_opportunities_filtered(
        &self,
        project_path: Option<&str>,
        limit: usize,
        target_savings_pct: f64,
    ) -> Result<Vec<GainOpportunity>> {
        ...
        let mut stmt = self.conn.prepare(
            "SELECT
                ...
                CASE
                    ...
                    THEN 100.0 * SUM(saved_tokens) / SUM(input_tokens)
                    ELSE 0.0
                END AS savings_pct,
                CAST(((SUM(input_tokens) * ?1) / 100.0) - SUM(saved_tokens) AS INTEGER) AS recoverable
             FROM commands
             WHERE input_tokens > 0
               AND (?2 IS NULL OR project_path = ?2 OR project_path GLOB ?3)
             GROUP BY original_cmd, rtk_cmd
             HAVING recoverable > 0
                AND (
                    saved = 0
                    OR savings_pct < 60.0
                    OR rtk_cmd LIKE '%proxy%'
                    OR rtk_cmd LIKE '%passthrough%'
                    OR rtk_cmd LIKE 'rtk fallback:%'
                )
        ...
        let rows = stmt.query_map(
            ...
            |row| {
                let original_cmd: String = row.get(0)?;
                let rtk_cmd: String = row.get(1)?;
                let saved_tokens = row.get::<_, i64>(5)? as usize;
                let savings_pct = row.get::<_, f64>(6)?;
                let (reason, suggested_action) =
                    opportunity_classification(&original_cmd, &rtk_cmd, saved_tokens, savings_pct);
                ...
                Ok(GainOpportunity {
                    ...
                    saved_tokens,
                    savings_pct,
                    recoverable_tokens: row.get::<_, i64>(7)? as usize,
                    reason,
                    suggested_action,
                })
            },
    ...
    pub fn ecosystem_mix(&self) -> Result<Vec<(String, f64)>> {
        ...
        let mut categories: HashMap<String,f64> =
            HashMap::new();
        ...
        let mut result: Vec<(String, f64)> = categories
            ...
            .collect();
        result.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(Equal));
        result.truncate(8);
        Ok(result)
    }
...
fn get_db_path() -> Result<PathBuf> {
    ...
    if let Ok(custom_path) = var("RTK_DB_PATH") {
...
mod tests {
    ...
    fn test_tracker() -> Tracker {
        ...
        Tracker::initialize_schema(&conn).expect("initialize schema");
        Tracker { conn }
    }
    ...
    fn test_custom_db_path_env() {
        ...
        unsafe { env::set_var("RTK_DB_PATH", &custom_path) };

        let db_path = get_db_path().expect("Failed to get db path");
        assert_eq!(db_path, custom_path);

        unsafe { env::remove_var("RTK_DB_PATH") };
    }
    ...
    // 14. opportunities rank recoverable tokens, not command names
    #[test]
    fn test_opportunities_rank_recoverable_tokens() {
        let tracker = test_tracker();
        insert_command(
            &tracker,
            "bash -lc rg foo src",
            "rtk proxy bash -lc rg foo src",
            "/repo",
            1000,
            1000,
        );
        ...
        insert_command(&tracker, "cargo test", "rtk cargo test", "/repo", 1000, 100);

        let opportunities = tracker
            .get_opportunities_filtered(None, 10, 80.0)
            .expect("opportunities");

        assert_eq!(opportunities.len(), 2);
        assert_eq!(opportunities[0].original_cmd, "bash -lc rg foo src");
        assert_eq!(opportunities[0].recoverable_tokens, 800);
        assert_eq!(opportunities[0].reason, "proxy");
        assert_eq!(opportunities[1].original_cmd, "git status --short");
        assert_eq!(opportunities[1].recoverable_tokens, 750);
    }

    // 15. opportunities respect project scoping
    #[test]
    fn test_opportunities_project_scope() {
        let tracker = test_tracker();
        insert_command(
            &tracker,
            "rg foo",
        ...
        let opportunities = tracker
            .get_opportunities_filtered(Some("/repo"), 10, 80.0)
            .expect("opportunities");

        assert_eq!(opportunities.len(), 1);
        assert_eq!(opportunities[0].original_cmd, "rg foo");
    }
}
