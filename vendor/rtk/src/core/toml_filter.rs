...
use crate::hooks::trust::TrustStatus::EnvOverride;
use crate::hooks::trust::TrustStatus::Trusted;
use crate::hooks::trust::TrustStatus::Untrusted;
use crate::hooks::trust::check_trust;
use std::path::Path;
...
impl TomlFilterRegistry {
    ...
    fn load() -> Self {
        ...
        let project_filter_path = Path::new(".rtk/filters.toml");
        if project_filter_path.exists() {
            let trust_status = check_trust(project_filter_path)
                .unwrap_or(Untrusted);

            match trust_status {
                Trusted
                | EnvOverride => {
                    if let Ok(content) = read_to_string(project_filter_path) {
...
pub fn run_filter_tests(filter_name_opt: Option<&str>) -> VerifyResults {
    ...
    let mut tested_filter_names: HashSet<String> =
        HashSet::new();
    ...
    let project_path = Path::new(".rtk/filters.toml");
    if project_path.exists() {
        let trust_status = check_trust(project_path)
            .unwrap_or(Untrusted);
        match trust_status {
            Trusted
            | EnvOverride => {
                if let Ok(content) = read_to_string(project_path) {
                    collect_test_outcomes(
                        &content,
                        filter_name_opt,
                        &mut outcomes,
...
fn collect_test_outcomes(
    ...
    tested_filter_names: &mut HashSet<String>,
) {
