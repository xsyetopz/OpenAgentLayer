...
pub fn print_with_hint(filtered: &str, raw: &str, tee_label: &str, exit_code: i32) {
    if let Some(hint) = tee_and_hint(raw, tee_label, exit_code) {
        println!("{}\n{}", filtered, hint);
    } else {
        println!("{}", filtered);
    }
...
pub fn run(
    ...
    match mode {
        ...
        RunMode::Streamed(filter) => {
            ...
            if let Some(label) = opts.tee_label {
                if let Some(hint) =
                    tee_and_hint(&result.raw, label, result.exit_code)
                {
                    println!("{}", hint);
                }
            }
...
pub fn run_passthrough(tool: &str, args: &[OsString], verbose: u8) -> Result<i32> {
    if verbose > 0 {
