use core::utils::exit_code_from_status;
use core::utils::resolved_command;
use std::process::Child;
use std::ptr::null_mut;
use core::tracking::TimedExecution;
use crate::core::config::Config;
use core::telemetry_cmd::run;
use core::config::show_config;
use core::telemetry::maybe_ping;
use std::process::exit;
use core::tracking::record_parse_failure_silent;
use core::toml_filter::apply_filter;
use core::tee::tee_and_hint;
use core::utils::exit_code_from_output;
use core::toml_filter::find_matching_filter;
use std::env::var;
use std::iter::once;
use core::utils::strip_ansi;
use core::telemetry_cmd::TelemetrySubcommand;
use core::filter::FilterLevel;
mod analytics;
mod cmds;
mod core;
mod discover;
...
enum Commands {
    ...
    Gain {
        ...
        #[arg(short = 'F', long)]
        failures: bool,
        /// Show commands with the most recoverable token savings
        #[arg(short = 'O', long)]
        opportunities: bool,
    },
...
fn run_fallback(parse_error: clap::Error) -> Result<i32> {
    ...
    let error_message = strip_ansi(&parse_error.to_string());
    ...
    let toml_match = if var("RTK_NO_TOML").ok().as_deref() == Some("1") {
        None
    } else {
        find_matching_filter(&lookup_cmd)
    };
    ...
    if let Some(filter) = toml_match {
        ...
        match result {
            Ok(output) => {
                let exit_code = exit_code_from_output(&output, &raw_command);
                ...
                let tee_hint = if !output.status.success() {
                    tee_and_hint(&combined_raw, &raw_command, exit_code)
                } else {
                    None
                };

                let filtered = apply_filter(filter, &combined_raw);
                println!("{}", filtered);
                ...
                record_parse_failure_silent(&raw_command, &error_message, true);

                Ok(exit_code)
            }
            Err(e) => {
                // Command not found — same behaviour as no-TOML path
                record_parse_failure_silent(&raw_command, &error_message, false);
                eprintln!("[rtk: {}]", e);
                Ok(127)
            }
        }
        ...
        let status = resolved_command(&args[0])
            .args(&args[1..])
            .stdin(std::process::Stdio::inherit())
        ...
        match status {
            Ok(s) => {
                ...
                Ok(exit_code_from_status(&s, &raw_command))
            }
            Err(e) => {
                record_parse_failure_silent(&raw_command, &error_message, false);
...
fn run_cli() -> Result<i32> {
    ...
    let code = match cli.command {
                    ...
                    if args.len() > 1 {
                        ...
                        match args[1].as_str() {
                            ...
                            _ => {
                                ...
                                exit_code_from_status(&status, "npx prisma")
                            }
                        }
                    } else {
                        let timer = TimedExecution::start();
                        let status = resolved_command("npx")
                            .arg("prisma")
                            .status()
        ...
        Commands::Proxy { args } => {
                ...
                unsafe extern "C" fn handle_signal(sig: libc::c_int) {
                    ...
                    if pid != 0 {
                        unsafe {
                            ...
                            libc::waitpid(pid as libc::pid_t, null_mut(), 0);
                        }
                    }
            ...
            exit_code_from_status(&status, &cmd_name)
        }
...
mod tests {
    ...
    #[test]
    fn test_gain_opportunities_flag_parses() {
        let result = Cli::try_parse_from(["rtk", "gain", "--opportunities"]);
        assert!(result.is_ok());
        if let Ok(cli) = result {
            match cli.command {
                Commands::Gain { opportunities, .. } => assert!(opportunities),
                _ => panic!("Expected Gain command"),
            }
        }
    }
