...
pub fn command_similarity(a: &str, b: &str) -> f64 {
    ...
    let args_a: HashSet<&str> = a
        .strip_prefix(&base_a)
        .unwrap_or("")
        .split_whitespace()
        .collect();

    let args_b: HashSet<&str> = b
        .strip_prefix(&base_b)
        .unwrap_or("")
        .split_whitespace()
        .collect();
...
fn extract_diff_token(wrong: &str, right: &str) -> String {
    let wrong_parts: HashSet<&str> = wrong.split_whitespace().collect();
    let right_parts: HashSet<&str> = right.split_whitespace().collect();
