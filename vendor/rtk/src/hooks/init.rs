...
fn migrate_old_hook_script(verbose: u8) {
    if let Some(home) = dirs::home_dir() {
        ...
        if old_hook.exists() {
            if let Err(e) = remove_file(&old_hook) {
                if verbose > 0 {
...
pub fn run_kilocode_mode(verbose: u8) -> Result<()> {
    run_kilocode_mode_at(&current_dir()?, verbose)
}
...
pub fn run_antigravity_mode(verbose: u8) -> Result<()> {
    run_antigravity_mode_at(&current_dir()?, verbose)
}
...
fn resolve_claude_dir() -> Result<PathBuf> {
    if let Ok(dir) = var("RTK_CLAUDE_DIR") {
...
fn resolve_codex_dir() -> Result<PathBuf> {
    resolve_codex_dir_from(
        var_os("CODEX_HOME").map(PathBuf::from),
        dirs::home_dir(),
    )
}
...
"#;
    ...
    fn with_claude_dir_override<F: FnOnce(&Path)>(tmp: &TempDir, f: F) {
        ...
        let orig = var_os("RTK_CLAUDE_DIR");
        unsafe { set_var("RTK_CLAUDE_DIR", &claude_dir) };
        f(&claude_dir);
        match orig {
            Some(v) => unsafe { set_var("RTK_CLAUDE_DIR", v) },
            None => unsafe { remove_var("RTK_CLAUDE_DIR") },
        }
    }
    ...
    fn test_local_init_no_hook() {
        ...
        set_current_dir(tmp.path()).unwrap();
