...
pub fn status() -> HookStatus {
    ...
    let Ok(content) = read_to_string(&hook_path) else {
...
fn binary_hook_registered(claude_dir: &Path) -> bool {
    ...
    let content = match read_to_string(&settings_path) {
...
fn check_and_warn() -> Option<()> {
    ...
    if let Ok(meta) = metadata(&marker) {
    ...
    let _ = create_dir_all(marker.parent()?);
    let _ = write(&marker, b"");
...
#[cfg(test)]
fn other_integration_installed(home: &Path) -> bool {
    let paths = [
...
mod tests {
    ...
    fn test_other_integration_empty_dirs_not_enough() {
        let tmp = tempfile::tempdir().expect("tempdir");
        create_dir_all(tmp.path().join(CURSOR_DIR).join(HOOKS_SUBDIR)).unwrap();
        create_dir_all(tmp.path().join(CODEX_DIR)).unwrap();
        create_dir_all(tmp.path().join(GEMINI_DIR)).unwrap();
        assert!(!other_integration_installed(tmp.path()));
    }
