...
use crate::core::utils::sha256_hex;
use std::fs::read;
use std::env::var;
...
fn canonical_key(filter_path: &Path) -> Result<String> {
    ...
    let canonical = canonicalize(filter_path)
...
pub fn check_trust(filter_path: &Path) -> Result<TrustStatus> {
    ...
    if var("RTK_TRUST_PROJECT_FILTERS").as_deref() == Ok("1") {
        let in_ci = var("CI").is_ok()
            || var("GITHUB_ACTIONS").is_ok()
            || var("GITLAB_CI").is_ok()
            || var("JENKINS_URL").is_ok()
            || var("BUILDKITE").is_ok();
        if in_ci {
...
pub fn run_trust(list: bool) -> Result<()> {
    ...
    let content_bytes = read(filter_path).context("Failed to read .rtk/filters.toml")?;
...
mod tests {
    ...
    fn check_trust_with_store(filter_path: &Path, store_file: &Path) -> Result<TrustStatus> {
        ...
        let store: TrustStore = if store_file.exists() {
            let content = read_to_string(store_file)?;
            serde_json::from_str(&content)?
        } else {
            TrustStore::default()
        };
    ...
    fn test_env_override_with_ci() {
        ...
        unsafe { set_var("RTK_TRUST_PROJECT_FILTERS", "1") };
        unsafe { set_var("CI", "true") };
        let status = check_trust(&filter).unwrap();
        unsafe { remove_var("RTK_TRUST_PROJECT_FILTERS") };
        unsafe { remove_var("CI") };
