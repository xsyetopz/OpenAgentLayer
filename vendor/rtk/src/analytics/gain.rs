...
use crate::core::tracking::{DayStats, GainOpportunity, MonthStats, Tracker, WeekStats};
...
pub fn run(
    ...
    if opportunities {
        return show_opportunities(&tracker, project_scope.as_deref(), format);
    }
...
fn resolve_project_scope(project: bool) -> Result<Option<String>> {
    ...
    let cwd = current_dir().context("Failed to resolve current working directory")?;
...
#[derive(Serialize)]
struct OpportunityExport {
    target_savings_pct: f64,
    opportunities: Vec<GainOpportunity>,
}
...
fn show_opportunities(tracker: &Tracker, project_scope: Option<&str>, format: &str) -> Result<()> {
    const LIMIT: usize = 20;
    const TARGET_SAVINGS_PCT: f64 = 80.0;

    let opportunities = tracker
        .get_opportunities_filtered(project_scope, LIMIT, TARGET_SAVINGS_PCT)
        .context("Failed to load token savings opportunities")?;

    match format {
        "json" => {
            let export = OpportunityExport {
        ...
        "csv" => {
            println!("original_cmd,rtk_cmd,count,input_tokens,output_tokens,saved_tokens,savings_pct,recoverable_tokens,reason,suggested_action");
            for opp in opportunities {
                println!(
                    "{},{},{},{},{},{},{:.2},{},{},{}",
                    csv_escape(&opp.original_cmd),
                    csv_escape(&opp.rtk_cmd),
                    opp.count,
                    opp.input_tokens,
                    opp.output_tokens,
                    opp.saved_tokens,
                    opp.savings_pct,
                    opp.recoverable_tokens,
                    csv_escape(&opp.reason),
                    csv_escape(&opp.suggested_action)
                );
            }
            return Ok(());
        }
    ...
    if opportunities.is_empty() {
        ...
        println!("Tracked commands already meet the opportunity threshold.");
        return Ok(());
    }

    println!("{}", styled("RTK Savings Opportunities", true));
    println!("{}", "═".repeat(72));
    print_kpi("Target savings", format!("{TARGET_SAVINGS_PCT:.0}%"));
    if let Some(scope) = project_scope {
        print_kpi("Scope", shorten_path(scope));
    }
    println!();

    let cmd_width = 30usize;
    let reason_width = 14usize;
    let action_width = 28usize;
    println!(
        "{:>3}  {:<cmd_width$}  {:>5}  {:>9}  {:>7}  {:>9}  {:<reason_width$}  {:<action_width$}",
        "#",
        "Command",
        "Count",
        "Recover",
    ...
    for (idx, opp) in opportunities.iter().enumerate() {
        println!(
            "{:>2}.  {:<cmd_width$}  {:>5}  {:>9}  {:>6.1}%  {:>9}  {:<reason_width$}  {:<action_width$}",
            idx + 1,
            truncate_for_column(&opp.original_cmd, cmd_width),
            opp.count,
            format_tokens(opp.recoverable_tokens),
            opp.savings_pct,
            format_tokens(opp.input_tokens),
            truncate_for_column(&opp.reason, reason_width),
            truncate_for_column(&opp.suggested_action, action_width),
            cmd_width = cmd_width,
            reason_width = reason_width,
            action_width = action_width
        );
    ...
    println!();
    println!("Recover = extra tokens saved if this command reaches the target.");
