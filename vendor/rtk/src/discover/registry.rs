...
fn append_redirect(command: String, redirect_suffix: &str) -> String {
    format!("{}{}", command, redirect_suffix)
}

fn format_tail(args: &[&str]) -> String {
    if args.is_empty() {
        String::new()
    } else {
...
fn rewrite_biome_command(args: &[&str], prefix: &str, redirect_suffix: &str) -> Option<String> {
    let subcommand = args.first().copied().unwrap_or("");
    let rest = join_args(args);
    let command = match subcommand {
        "lint" => format!("{}rtk lint biome {}", prefix, rest),
        "format" => format!("{}rtk format biome {}", prefix, rest),
        "check" => format!("{}rtk summary biome {}", prefix, rest),
        _ => return None,
    };
    Some(append_redirect(
        command.trim_end().to_string(),
        redirect_suffix,
    ))
}

fn rewrite_bun_command(cmd: &str, prefix: &str, redirect_suffix: &str) -> Option<String> {
    let tokens: Vec<&str> = cmd.split_whitespace().collect();
    match tokens.as_slice() {
        ["bun", "test", rest @ ..] => Some(append_redirect(
            format!("{}rtk test bun test{}", prefix, format_tail(rest)),
            redirect_suffix,
        )),
        ["bun", "run", script, rest @ ..] if script.starts_with("test") => Some(append_redirect(
            format!("{}rtk test bun run {}{}", prefix, script, format_tail(rest)),
            redirect_suffix,
        )),
        ["bun", "run", script, rest @ ..]
            if matches!(*script, "typecheck" | "type-check" | "tsc") =>
        {
            let tsc_args = if rest.is_empty() {
                "--noEmit".to_string()
            } else {
                join_args(rest)
            };
            Some(append_redirect(
                format!("{}rtk tsc {}", prefix, tsc_args),
                redirect_suffix,
            ))
        }
        ["bun", "run", script, rest @ ..] if matches!(*script, "lint") => Some(append_redirect(
            format!("{}rtk lint bun run {}{}", prefix, script, format_tail(rest)),
            redirect_suffix,
        )),
        ["bun", "run", script, rest @ ..]
            if matches!(*script, "build" | "compile" | "check" | "validate") =>
        {
            Some(append_redirect(
                format!(
                    "{}rtk summary bun run {}{}",
        ...
        ["bunx", "tsc", rest @ ..] => Some(append_redirect(
            format!("{}rtk tsc{}", prefix, format_tail(rest)),
            redirect_suffix,
        )),
        ["bunx", "biome", subcommand, rest @ ..] => rewrite_biome_command(
            &[*subcommand]
                .into_iter()
        ...
        ["bunx", "--cwd", dir, tool, rest @ ..] => {
            let inner = match *tool {
                "tsc" => format!("rtk tsc{}", format_tail(rest)),
                "biome" => {
                    let [subcommand, biome_rest @ ..] = rest else {
                        return None;
                    };
                    rewrite_biome_command(
                        &[*subcommand]
                            .into_iter()
                            .chain(biome_rest.iter().copied())
                            .collect::<Vec<_>>(),
                        "",
                        "",
                    )?
            ...
            let command = format!("cd {} && {}", shell_quote(dir), inner);
            Some(append_redirect(
                format!("{}rtk proxy -- bash -lc {}", prefix, shell_quote(&command)),
                redirect_suffix,
            ))
        }
        _ => None,
...
fn rewrite_simple_command(cmd: &str, prefix: &str, redirect_suffix: &str) -> Option<String> {
    if let Some(rewritten) = rewrite_bun_command(cmd, prefix, redirect_suffix) {
        return Some(rewritten);
    }

    let tokens: Vec<&str> = cmd.split_whitespace().collect();
    match tokens.as_slice() {
        ["flutter", "test", rest @ ..] => Some(append_redirect(
            format!("{}rtk test flutter test{}", prefix, format_tail(rest)),
            redirect_suffix,
        )),
        ["flutter", "analyze", rest @ ..] => Some(append_redirect(
            format!("{}rtk summary flutter analyze{}", prefix, format_tail(rest)),
            redirect_suffix,
        )),
        ["adb", "logcat", rest @ ..]
        | ["adb", "shell", rest @ ..]
        | ["adb", "install", rest @ ..] => Some(append_redirect(
            format!(
                "{}rtk summary adb {}{}",
                prefix,
                tokens[1],
        ...
        ["sbt", "test", rest @ ..] => Some(append_redirect(
            format!("{}rtk test sbt test{}", prefix, format_tail(rest)),
            redirect_suffix,
        )),
        ["jq", query, file] if matches!(*query, "." | "-S") && file.ends_with(".json") => Some(
            append_redirect(format!("{}rtk json {}", prefix, file), redirect_suffix),
        ),
        ["jq", "-r", _query, file] if file.ends_with(".json") => Some(append_redirect(
            format!("{}rtk json {}", prefix, file),
            redirect_suffix,
        )),
        ["env"] | ["printenv"] => Some(append_redirect(
            format!("{}rtk env", prefix),
            redirect_suffix,
        )),
        ["sed", "-n", range, file] => rewrite_sed_range(range, file)
            .map(|rewritten| append_redirect(format!("{}{}", prefix, rewritten), redirect_suffix)),
        _ => None,
    }
}

fn rewrite_sed_range(range: &str, file: &str) -> Option<String> {
    let stripped = range.trim_matches('"').trim_matches('\'');
    let Some(max) = stripped
        .strip_prefix("1,")
    ...
    if max.chars().all(|c| c.is_ascii_digit()) {
        Some(format!("rtk read --max-lines {} {}", max, file))
    } else {
        None
    }
}
...
fn rewrite_segment_inner(seg: &str, excluded: &[ExcludePattern], depth: usize) -> Option<String> {
    ...
    if let Some(rewritten) = rewrite_simple_command(cmd_clean, env_prefix, redirect_suffix) {
        return Some(rewritten);
    }
...
mod tests {
    ...
    fn test_rewrite_rtk_disabled_subprocess_warns() {
        let rtk_bin = Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("target")
            .join("debug")
            .join("rtk");
        if !rtk_bin.exists() {
        ...
        let rtk_mtime = metadata(&rtk_bin)
            .ok()
            .and_then(|m| m.modified().ok());
        let test_mtime = current_exe()
            .ok()
            .and_then(|p| metadata(p).ok())
            .and_then(|m| m.modified().ok());
    ...
    #[test]
    fn test_rewrite_bun_and_bunx_commands() {
        assert_eq!(
            rewrite_command("bun test tests", &[]),
            Some("rtk test bun test tests".into())
        );
        assert_eq!(
            rewrite_command("bun run typecheck", &[]),
            Some("rtk tsc --noEmit".into())
        );
        assert_eq!(
        ...
        assert_eq!(
            rewrite_command("bunx biome check source tests", &[]),
            Some("rtk summary biome check source tests".into())
        );
    }

    #[test]
    fn test_rewrite_bunx_cwd_commands() {
        assert_eq!(
            rewrite_command("bunx --cwd opencode tsc --noEmit", &[]),
            Some("rtk proxy -- bash -lc 'cd '\\''opencode'\\'' && rtk tsc --noEmit'".into())
        );
    }

    #[test]
    fn test_rewrite_mobile_and_jvm_commands() {
        assert_eq!(
            rewrite_command("flutter test", &[]),
            Some("rtk test flutter test".into())
        );
        assert_eq!(
            rewrite_command("flutter analyze", &[]),
            Some("rtk summary flutter analyze".into())
        );
        assert_eq!(
            rewrite_command("adb logcat", &[]),
            Some("rtk summary adb logcat".into())
        );
        assert_eq!(
            rewrite_command("sbt test", &[]),
            Some("rtk test sbt test".into())
        );
    }

    #[test]
    fn test_rewrite_system_helpers() {
        assert_eq!(
            rewrite_command("jq . package.json", &[]),
            Some("rtk json package.json".into())
        );
        assert_eq!(rewrite_command("env", &[]), Some("rtk env".into()));
        assert_eq!(
            rewrite_command("sed -n 1,5p README.md", &[]),
            Some("rtk read --max-lines 5 README.md".into())
        );
    }

    #[test]
    fn test_rewrite_build_tool_rules() {
        assert_eq!(
            rewrite_command("dotnet format Osu.sln", &[]),
            Some("rtk dotnet format Osu.sln".into())
        );
        assert_eq!(
            rewrite_command("./gradlew test", &[]),
            Some("rtk gradle test".into())
        );
        assert_eq!(
            rewrite_command("xcodebuild -scheme App build", &[]),
