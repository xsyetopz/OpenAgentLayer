...
fn run_aws_filtered(
    ...
    if !status.success() {
        ...
        if let Some(hint) = tee_and_hint(&raw, &slug, exit_code) {
            eprintln!("{}\n{}", stderr.trim(), hint);
        } else {
            eprintln!("{}", stderr.trim());
        }
    ...
    } else if let Some(hint) = tee_and_hint(&raw, &slug, 0) {
        println!("{}\n{}", result.text, hint);
    } else {
        println!("{}", result.text);
    }
...
fn run_s3_ls(extra_args: &[String], verbose: u8) -> Result<i32> {
    ...
    if !output.status.success() {
        ...
        if let Some(hint) = tee_and_hint(&raw, "aws_s3_ls", exit_code) {
            eprintln!("{}\n{}", stderr.trim(), hint);
        } else {
            eprintln!("{}", stderr.trim());
        }
...
fn filter_sqs_messages(json_str: &str) -> Option<FilterResult> {
    ...
    for msg in messages.iter().take(MAX_ITEMS) {
        ...
        let body_truncated = truncate(body, 200);
