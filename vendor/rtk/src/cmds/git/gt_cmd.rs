...
use crate::core::runner::run_passthrough;
use crate::git::GitCommand::Worktree;
use crate::git::run;
use crate::git::GitCommand::Fetch;
use crate::git::GitCommand::Pull;
use crate::git::GitCommand::Push;
use crate::git::GitCommand::Add;
use crate::git::GitCommand::Show;
use crate::git::GitCommand::Diff;
use crate::git::GitCommand::Status;
use crate::core::tee::tee_and_hint;
...
fn run_gt_filtered(
    ...
    if let Some(hint) = tee_and_hint(&raw, tee_label, cmd_output.exit_code) {
        println!("{}\n{}", output, hint);
    } else {
        println!("{}", output);
    }
...
pub fn run_other(args: &[OsString], verbose: u8) -> Result<i32> {
    ...
    match subcommand.as_ref() {
        "status" => run(Status, &rest, None, verbose, &[]),
        "diff" => run(Diff, &rest, None, verbose, &[]),
        "show" => run(Show, &rest, None, verbose, &[]),
        "add" => run(Add, &rest, None, verbose, &[]),
        "push" => run(Push, &rest, None, verbose, &[]),
        "pull" => run(Pull, &rest, None, verbose, &[]),
        "fetch" => run(Fetch, &rest, None, verbose, &[]),
        "stash" => {
        ...
        "worktree" => run(Worktree, &rest, None, verbose, &[]),
...
fn passthrough_gt(subcommand: &str, args: &[String], verbose: u8) -> Result<i32> {
    ...
    run_passthrough("gt", &os_args, verbose)
}
