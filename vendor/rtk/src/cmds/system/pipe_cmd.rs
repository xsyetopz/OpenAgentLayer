...
use crate::cmds::python::mypy_cmd::filter_mypy_output;
use crate::cmds::python::pytest_cmd::filter_pytest_output;
use crate::cmds::rust::cargo_cmd::filter_cargo_test;
use crate::cmds::git::git::compact_diff;
use crate::cmds::git::git::filter_log_output;
use crate::cmds::go::go_cmd::filter_go_test_json;
use crate::cmds::js::prettier_cmd::filter_prettier_output;
use crate::cmds::python::ruff_cmd::filter_ruff_format;
use crate::cmds::python::ruff_cmd::filter_ruff_check_json;
use crate::cmds::git::git::format_status_output;
use crate::cmds::js::tsc_cmd::filter_tsc_output;
use crate::cmds::go::go_cmd::filter_go_build;
use anyhow::Result;
use std::io::Read;
...
pub fn resolve_filter(name: &str) -> Option<fn(&str) -> String> {
    match name {
        "cargo-test" | "cargo" => Some(filter_cargo_test),
        "pytest" => Some(filter_pytest_output),
        "go-test" => Some(go_test_wrapper),
        "go-build" => Some(filter_go_build),
        "tsc" => Some(filter_tsc_output),
        ...
        "git-status" => Some(format_status_output),
        "mypy" => Some(filter_mypy_output),
        "ruff-check" => Some(filter_ruff_check_json),
        "ruff-format" => Some(filter_ruff_format),
        "prettier" => Some(filter_prettier_output),
        _ => None,
    }
}

fn go_test_wrapper(input: &str) -> String {
    filter_go_test_json(input)
}

fn git_log_wrapper(input: &str) -> String {
    filter_log_output(input, 50, false, false)
}

fn git_diff_wrapper(input: &str) -> String {
    compact_diff(input, 200)
}
...
pub fn auto_detect_filter(input: &str) -> fn(&str) -> String {
    ...
    if first_1k.contains("test result:") && first_1k.contains("passed;") {
        return filter_cargo_test;
    }

    if first_1k.contains("=== test session starts") {
        return filter_pytest_output;
    }
...
fn apply_filter(filter_fn: fn(&str) -> String, input: &str) -> String {
    catch_unwind(AssertUnwindSafe(|| filter_fn(input)))
        .unwrap_or_else(|_| {
