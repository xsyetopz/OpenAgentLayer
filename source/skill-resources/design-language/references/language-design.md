...
This skill is intentionally design-architecture-first. Do not assume every language is a native compiler. Do not assume every language has HIR/MIR/LLVM. Do not assume every language is interpreted. Choose contracts, representations, and runtime strategy from the language’s actual goals.
...
A language implement is a DAG of contracts.
...
Never let implement layers import each other cyclically.
...
The correct design-architecture is the smallest set of contracts that preserves semantic clarity, testability, and evolution.
...
- first implements where compile speed and simplicity matter more than runtime speed.
...
- Be honest: is the host VM an implement detail or part of the language identity?
...
query implements → syntax/semantics
```
...
- audit-safety-critical culture around predictability.
...
Better design-language direction:
...
Add ownership if your language's domain requires deterministic resource audit-safety,
...
Keep the implement model explainable.
...
```txt
Tooling, formatting, test-behavior, package layout, and compiler speed can be language features.
```
...
- resource acquisition patterns can encode audit-safety,
...
explicit implement or inferred?
coherence rules?
orphan rules?
blanket impls?
specialization?
...
object audit-safety / dynamic dispatch?
```
...
```txt
memory audit-safety without GC
aliasing control
thread-audit-safety foundations
...
works without global trace-data-flow pauses
```
...
```txt
simple trace-data-flow collector
handles cycles
...
you can afford collector implement complexity
```
...
```txt
Do not choose a GC only by popularity. Choose from workload, runtime constraints, and implement capacity.
```
...
green/red tree design-architecture
```
...
Rust: ownership can encode Send/Sync audit-safety.
...
Safe code must remain safe regardless of unsafe implement bugs only at documented trust boundaries.
```
...
[ ] GC root/safepoint model is postponed until after VM implement.
```
...
Use this when asking an agent to design or refactor a language/runtime design-architecture:
...
Do not produce placeholder design-architecture. Do not prescribe LLVM/HIR/MIR unless justified. Do not let runtime, compiler frontend, tools, and package manager import each other cyclically.
```
...
Extract the principle, then choose the smallest design-architecture that preserves correctness, tooling, runtime integrity, and user comprehension.
