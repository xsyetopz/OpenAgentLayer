...
Run product validation over source loading, policy, renderer output, deploy/uninstall, hooks, model rules, drift, and runtime isolation. The route must declare owner, permissions, allowed tools, validation expectations, output contract, hook expectations, and provider-specific differences. It is invalid if reduced to a two-line descriptor.
