#!/bin/bash
# auto-index.sh - Automatic index management for Claude Code agent team
#
# Usage:
#   ./auto-index.sh check          - Check if index needs refresh
#   ./auto-index.sh mark-dirty     - Mark index as potentially stale
#   ./auto-index.sh cleanup-locks  - Remove stale locks
...
# Maximum age of index before suggesting refresh (in seconds)
MAX_INDEX_AGE=86400  # 24 hours
...
cleanup_locks() {
    ...
    local lock_timeout=3600  # 1 hour

    # Create temp file for new locks
    local temp_file=$(mktemp)

    # Keep header and non-expired locks
    local header_done=false
    while IFS= read -r line; do
        # Keep header lines (start with | and contain "File" or start with |---)
