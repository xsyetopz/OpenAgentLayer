...
function getGitContext() {
  const parts = [];
  try {
    const branch = spawnSync('git', ['rev-parse', '--abbrev-ref', 'HEAD'], { encoding: 'utf8', timeout: 1000 });
    if (branch.status === 0 && branch.stdout.trim()) {
      parts.push(`Branch: ${branch.stdout.trim()}`);
    }

    const log = spawnSync('git', ['log', '--oneline', '-5', '--no-decorate'], { encoding: 'utf8', timeout: 1000 });
    if (log.status === 0 && log.stdout.trim()) {
      parts.push(`Recent commits:\n${log.stdout.trim()}`);
    }

    const status = spawnSync('git', ['diff', '--stat', '--no-color', 'HEAD'], { encoding: 'utf8', timeout: 1000 });
