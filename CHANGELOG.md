...
| ---------- | ------------------------- | ------------------------ | ---------------------------- | --------------------------- |
| `go`       | `gpt-5.5-mini`            | `medium`                 | `high`                       | `gpt-5.3-codex`             |
| `plus`     | `gpt-5.5`                 | `medium`                 | `high`                       | `gpt-5.3-codex`             |
| `pro-5`    | `gpt-5.5`                 | `medium`                 | `high`                       | `gpt-5.3-codex`             |
| `pro-20`   | `gpt-5.5`                 | `medium`                 | `high`                       | `gpt-5.3-codex`             |
...
- Codex sample config and managed guidance are aligned to the `gpt-5.4` / `gpt-5.3-codex` / `gpt-5.4-mini` split (no managed `gpt-5.5` routing).
...
- Codex model presets and routing:
  - the main high-reasoning route now defaults to `gpt-5.4`.
  - the implementation-heavy route stays on `gpt-5.3-codex`.
  - the utility route now uses `gpt-5.4-mini`.
  - the stable `openagentsbtw` profile is pinned to `gpt-5.4` instead of mirroring the selected preset.
- Codex install defaults now prefer the current high-reasoning preset when no explicit Codex plan is supplied.
- Managed Codex guidance and research docs now describe the `gpt-5.4` / `gpt-5.3-codex` / `gpt-5.4-mini` split and remove full `gpt-5.5` from managed routing.
...
- Codex model presets and routing:
  ...
  - the flagship main route defaults to `gpt-5.5`.
...
- Codex defaults now follow a 5.2-first routing policy for day-to-day work:
  - `gpt-5.4` for planning and review-oriented paths
  - `gpt-5.4-codex` for implementation paths
  - `gpt-5.5` kept only for the explicit higher-tier planning/orchestration path
