...
- For `plus`, `pro-5`, and `pro-20`, top-level `openagentsbtw` defaults to `gpt-5.5` with `model_reasoning_effort = "medium"` and `plan_mode_reasoning_effort = "high"`. `go` keeps `gpt-5.5-mini` at top-level.
- Implementation/autopilot/long-runtime profiles stay on `gpt-5.3-codex`.
- `openagentsbtw-utility`
  A lightweight profile for bounded utility tasks. It uses `gpt-5.5-mini`.
