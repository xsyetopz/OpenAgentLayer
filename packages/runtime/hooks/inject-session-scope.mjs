...
const SESSION_SCOPE_DETAILS = [
	...
	"Agent use: use OpenDex/Symphony as the Codex default orchestration path. Native Codex multi_agent or multi_agent_v2 is explicit profile opt-in only; when enabled, spawn rendered OAL custom agents by name, wait for their results, and merge evidence in the parent thread. Use `opendex`, `oal opendex`, or Symphony workflow commands for bounded orchestration checks.",
